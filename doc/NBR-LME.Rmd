...
Here we can obtain the correponding pairwise interaction of the brain network for each edge.
...
Although five permutations is quite low to obtain a proper null distribution, we can see that they take several seconds to be performed. So we suggest parallelizing to multiple CPU cores with the `cores` argument.
...
If we observed the Family-Wise Error (FWE) probabilities of the observed components, only the component 1 in the `Session` term is lower than the nominal alpha of p < 0.05. The table shows the probabilites associated with: 1) the number of connected eges, and 2) the sum of the strength of the edges. In this case, we will use the sum of strengths, but you can choose depending on your research question.
