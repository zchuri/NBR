#' @title Network-based R-statistics using Mixed Effects Models ANOVA
#'
#' @description This function computes the specified (non)linear mixed models (LME) ANOVA for
#'  each edge in the network, and calculates the family wise error (FWE) p-value for the size
#'  of the clusters of connected edges that are individually below the P threshold
#'  (\emph{thrP}), or above the F threshold (\emph{thrF}). FWE estimation is based on the null
#'  distribution of the maximum size of sets of connected edges (defined as above), obtained
#'  with \emph{nperm} permutations of the original data.
#'
#' @usage nbr_lme_aov(net, nnodes, idata, mod, rdm, diag = FALSE,
...
nbr_lme_aov <- function(net,
  ...
  fit_ex <- tryCatch(anova(lme(lm_form, idata, as.formula(rdm), ...)),
                     error=function(e) NA)
  ...
  nedges <- nrow(tri_pos)
  nvars <- nrow(fit_ex)-1
  obsF <- matrix(as.numeric(NA), nrow = nedges, ncol = nvars*2)
  ...
  if(is.null(cores)){
    ...
    # Apply example model edgewise
    obsF <- t(parSapply(cl, 1:nedges, function(x){
      lm_form <- as.formula(paste0(names(idata)[idata_dim[2]+x], mod))
      fit <- tryCatch(anova(lme(lm_form, idata, as.formula(rdm), ...)),
                      error=function(e) NA)
      # Store NA if individual model does not fit
  ...
  colnames(obsF) <- paste0(rep(rownames(fit_ex)[1:nvars+1], each = 2),c("_F","_p"))
  if(verbose) cat(".\n")
  ...
  if(thr_idx == 1){
    ...
    for(ii in 1:nvars){
      ...
      if(length(edges)>0){
        ...
        component <- vector("integer", length(edges))
        # Store strength intensity
        strength <- obsF[edges,ii*thr_idx-1]-qf(1-thrP,fit_ex$numDF[ii+1],fit_ex$denDF[ii+1])
        # Label nodes and edge component
        for(jj in 1:length(edges)){
          # Maximum label to be changed
          max_lab <- max(obs_comp[tri_pos[edges[jj],],ii])
  ...
  for(pp in 1:nperm){
    ...
    # Create empty object to store F-statistic and p-values edgewise
    permF <- matrix(as.numeric(NA), nrow = nedges, ncol = nvars*2)
    ...
    if(is.null(cores)){
      # Apply example model edgewise
      permF <- t(sapply(1:nedges, function(x){
        lm_form <- as.formula(paste0(names(pdata)[idata_dim[2]+x], mod))
        fit <- tryCatch(anova(lme(lm_form, pdata, as.formula(rdm), ...)),
                        error=function(e) NA)
        # Store NA if individual model does not fit
        if(all(is.na(as.matrix(fit)))){
          return(rep(NA,nvars*2))
      ...
      clusterExport(cl=cl, varlist=vlist, envir=environment())
      # Apply example model edgewise
      permF <- t(parSapply(cl, 1:nedges, function(x){
        lm_form <- as.formula(paste0(names(pdata)[idata_dim[2]+x], mod))
        fit <- tryCatch(anova(lme(lm_form, pdata, as.formula(rdm), ...)),
                        error=function(e) NA)
        # Store NA if individual model does not fit
    ...
    if(thr_idx == 1){
      ...
      for(ii in 1:nvars){
        ...
        if(length(edges)>0){
          ...
          component <- vector("integer", length(edges))
          # Store strength
          strength <- permF[edges,ii*thr_idx-1]-qf(1-thrP,fit_ex$numDF[ii+1],fit_ex$denDF[ii+1])
          # Label nodes and edge component
          for(jj in 1:length(edges)){
            # Maximum label to be changed
            max_lab <- max(perm_comp[tri_pos[edges[jj],],ii])
